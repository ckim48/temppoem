...
def is_acroustic(theme, poem):
    ...
    if not match_theme(theme,poem):
        return False
    if isSimillar_theme(theme, poem) < threshold:
        return False
    return True
...
def isSimillar_theme(theme, lines): #lines = ["bring out your bat","and your ball..",",,,,"]
    poem = " ".join(lines)
    poem = poem.lower()
    cleaned_poem = re.sub("[^\w\s]", "", poem) # \w --> alphabets, digits, _. \s --> white space
    # Hello my name is scott
    list_words = cleaned_poem.split() # ["Hello", "my", "name",....]
    stopword_list = stopwords.words('english')  # getting english topwords

    # no_stopword -> it contains the words in poem, but not stopwords
    no_stopword = [word for word in list_words if word not in stopword_list]
    similarity_score = 0
    number_word = 0
